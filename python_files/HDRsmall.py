...
class HDRnet(torch.nn.Module):
    def __init__(self, n_hidden_neurons=64):
        ...
        self.decode0 = torch.nn.Sequential(
            torch.nn.Upsample(size=(135, 240), mode='bilinear',
                              align_corners=False),
            torch.nn.Conv2d(256, 128, kernel_size=1, stride=1, bias=False),
            torch.nn.BatchNorm2d(128),
            torch.nn.LeakyReLU()
        )
        # CONCATENATE HERE
        # 2
        self.decode1 = UpSample(256)
        self.decode2 = UpSample(128, div_factor=1)
    ...
    def forward(self, sup0):
        ...
        x = self.decode0(x)
        x = torch.cat((x, sup3), dim=1)

        x = self.decode1(x, sup2)
        x = self.decode2(x, sup1)
